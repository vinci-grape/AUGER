...
ROOT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), './RWB/repo')
# random seed
RAND_SEED=1234

def repo_path(proj, bugid):
    return os.path.join(ROOT_DIR, f'{proj}/{bugid}')

class TimeoutException(Exception):
    pass

def path_prefix(proj, bug_num):
    if proj == 'Chart':
        return 'source/'
    elif proj == 'Closure':
        return 'src/'
...
def test_path_prefix(proj, bug_num):
    if proj == 'Chart':
        return 'tests/'
    elif proj == 'Closure':
        return 'test/'

...
class PASTA(abc.ABC):
    ...
    def token_ranges_from_batch(
        ...
        result = [ 
                [tokenizer_utils.find_token_range(
                    string, emphasis, offset_mapping=offset_mapping, occurrence=occurrence
                ) for emphasis in substring]
    ...
    def edit_attention_mask(
        ...
        for bi, token_range in enumerate(token_range):
            mask = torch.ones(attention_mask.size(3), dtype=torch.bool)
            for range_item in token_range:
                if range_item is not None:
                    ti, tj = range_item
                    mask[ti:tj] = False  
            # Use the boolean mask to update attention_mask in a vectorized way
            for idx in head_idx:
                attention_mask[bi, idx, :, mask] += scale_constant
